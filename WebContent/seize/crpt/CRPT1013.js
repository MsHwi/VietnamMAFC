...
corebase.wrapping("#CRPT1013", function($cbwrap, $cbfind, corebase) {
	...
	function fn_srch() {
	    ...
	    corebase.selectList ( source, "CRPT1013.SELECT_AGENT_PERF", function ( output, metadata ){
	...
	function fn_exptFile(type) {
	    ...
	    detail.push(cb.locMessage("txt.1428", "Campaign") + " : " + $cbfind("#DIV_SRCH #CMP_ID").cbText());
	    detail.push(cb.locMessage("txt.972", "Organization") + " : " + $cbfind("#DIV_SRCH #CMB_CNTR_CD").cbText() + " / " + $cbfind("#DIV_SRCH #CMB_GRP_CD").cbText() + " / " + $cbfind("#DIV_SRCH #CMB_TEAM_CD").cbText() + " / "+ $cbfind("#DIV_SRCH #CMB_USR_ID").cbText());
	    ...
	    data.SELECT_AGENT_PERF = corebase.Input("selectList", "CRPT1013.SELECT_AGENT_PERF", source, header); // select query
